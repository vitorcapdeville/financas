...
const mockAplicarTodasRegrasAction =
  aplicarTodasRegrasAction as jest.MockedFunction<
    typeof aplicarTodasRegrasAction
  >;

describe("BotaoAplicarTodasRegras", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockAplicarTodasRegrasAction.mockResolvedValue({
  ...
  it("não deve mostrar modal inicialmente", () => {
    ...
    expect(
      screen.queryByText("Aplicar Todas as Regras")
  ...
  it("deve exibir mensagem de confirmação no modal", () => {
    ...
    expect(
      screen.getByText(/TODAS as transações existentes/i)
    ).toBeInTheDocument();
  });
  ...
  it("deve exibir toast de sucesso com número de aplicações", async () => {
    ...
    await waitFor(() => {
      expect(toast.success).toHaveBeenCalledWith(
        expect.stringContaining("25 de 100 transações modificadas")
      );
    });
  });
  ...
  it("deve exibir toast de erro quando falha", async () => {
    mockAplicarTodasRegrasAction.mockRejectedValue(
      new Error("Erro ao aplicar")
    );
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith(
        expect.stringContaining("Erro ao aplicar regras")
      );
    });
  });
  ...
  it("deve fechar modal ao cancelar", () => {
    ...
    expect(
      screen.queryByText(/TODAS as regras ativas/i)
    ).not.toBeInTheDocument();
  });
